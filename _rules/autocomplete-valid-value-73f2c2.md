...
- **fixed value**: the element is an `input` element with a `type` [attribute value][] of `button`, `checkboc`, `file`, `image`, `radio`, `reset` or `submit`; or
...
The intent of [fixed value] in Applicability section is to clearly define which `input` element's `type` attribute [attribute value][] apply to the element.
